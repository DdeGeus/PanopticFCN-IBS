...
# Adapted from detectron2/data/dataset_mapper.py by Daan de Geus
...
__all__ = ["MapillaryVistasPanopticDatasetMapper"]


class MapillaryVistasPanopticDatasetMapper:
  """
  ...
  def __call__(self, dataset_dict):
    ...
    #
    # sem_seg_gt = mapper[sem_seg_gt_tmp]

    boxes = np.array(boxes)
    if not len(boxes) == 0:
      boxes = BoxMode.convert(boxes, BoxMode.XYWH_ABS, BoxMode.XYXY_ABS)

    aug_input = AugInput(image, pan_seg=pan_seg_gt.astype(np.float32), boxes=boxes)
    transforms = self.augmentations(aug_input)
    image, pan_seg_gt = aug_input.image, aug_input.pan_seg.astype(np.int32)

    # aug_input = AugInput(image, sem_seg=sem_seg_gt, pan_seg=pan_seg_gt.astype(np.float32), boxes=boxes)
    # transforms = self.augmentations(aug_input)
    # image, sem_seg_gt, pan_seg_gt = aug_input.image, aug_input.sem_seg, aug_input.pan_seg.astype(np.int32)
