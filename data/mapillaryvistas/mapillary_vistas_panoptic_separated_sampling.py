...
def load_mapillary_vistas_panoptic_sampling(images_per_class_json):
...
_RAW_MAPILLARY_VISTAS_PANOPTIC_SPLITS = {
    "mapillary_vistas_panoptic_train_separated": (
        ...
        "mapillary-vistas/training/images_per_class.json",
    ),
...
def register_all_mapillary_vistas_panoptic_sampling(cfg):
    ...
    images_per_class_json = images_dir.replace("images", "images_per_class.json")
    meta["images_per_class_json"] = os.path.join(root, images_per_class_json)

    json_per_img_dir = images_dir.replace("images", "panoptic_json/")
    ...
    for key in _RAW_MAPILLARY_VISTAS_PANOPTIC_SPLITS.keys():
        if "panoptic_val" in key:
            (image_dir, gt_dir, gt_json) = _RAW_MAPILLARY_VISTAS_PANOPTIC_SPLITS[key]
            ...
            (image_dir, gt_dir, gt_json, json_dir) = _RAW_MAPILLARY_VISTAS_PANOPTIC_SPLITS[key]
            image_dir = os.path.join(root, image_dir)
            gt_dir = os.path.join(root, gt_dir)
            gt_json = os.path.join(root, gt_json)
            ...
            DatasetCatalog.register(
                key, lambda x=json_dir: load_mapillary_vistas_panoptic_sampling(x)
            )
            MetadataCatalog.get(key).set(
                panoptic_root=gt_dir,
                image_root=image_dir,
