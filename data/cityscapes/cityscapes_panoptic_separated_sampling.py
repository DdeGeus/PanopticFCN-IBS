...
def load_cityscapes_panoptic_newsampling(images_per_class_json):
    with open(images_per_class_json, 'r') as fp:
        images_per_class_dict = json.load(fp)

    cat_id_list = images_per_class_dict['cat_ids']
    ret = list()
    for cat_id in cat_id_list:
        ret.append({'cat_id': cat_id})
...
_RAW_CITYSCAPES_PANOPTIC_SPLITS = {
    "cityscapes_fine_panoptic_train_separated": (
        ...
        "cityscapes/gtFine/cityscapes_panoptic_train",
        "cityscapes/gtFine/cityscapes_panoptic_train.json",
        "cityscapes/images_per_class_train.json"
    ),
...
def register_all_cityscapes_panoptic_newsampling(cfg):
    ...
    images_dir = _RAW_CITYSCAPES_PANOPTIC_SPLITS["cityscapes_fine_panoptic_train_separated"][0]

    images_per_class_json = images_dir.replace("leftImg8bit/train", "images_per_class_train.json")
    meta["images_per_class_json"] = os.path.join(root, images_per_class_json)

    json_per_img_dir = images_dir.replace("leftImg8bit/train", "panoptic_json_train/")
    meta["json_per_img_dir"] = os.path.join(root, json_per_img_dir)
    ...
    for key in _RAW_CITYSCAPES_PANOPTIC_SPLITS.keys():
        if 'val' in key:
            image_dir, gt_dir, gt_json = _RAW_CITYSCAPES_PANOPTIC_SPLITS[key]
            ...
            image_dir, gt_dir, gt_json, images_per_cat_json = _RAW_CITYSCAPES_PANOPTIC_SPLITS[key]

            image_dir = os.path.join(root, image_dir)
            gt_dir = os.path.join(root, gt_dir)
            gt_json = os.path.join(root, gt_json)
            images_per_cat_json = os.path.join(root, images_per_cat_json)

            DatasetCatalog.register(
                key, lambda x=images_per_cat_json: load_cityscapes_panoptic_newsampling(x)
            )
            MetadataCatalog.get(key).set(
                panoptic_root=gt_dir,
                image_root=image_dir,
